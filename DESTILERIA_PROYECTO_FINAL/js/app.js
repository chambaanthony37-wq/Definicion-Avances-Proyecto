...
class App {
    ...
    init() {
        ...
        this.controllers.auth.init((usuario) => {
            // Si cambia el usuario (o es el primer login), se recargan todos los datos
            if (this.usuarioActualCache !== usuario.id) {
                this.usuarioActualCache = usuario.id;
                this.cargarDatos();
            } else {
                // Si es el mismo usuario, solo recargamos sus mensajes por si acaso
